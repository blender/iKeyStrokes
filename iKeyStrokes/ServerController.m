...
-(void) startService
{
    ...
    NSString *serviceName = [NSString stringWithFormat:@"iKeyStrokes on %@", [[NSProcessInfo processInfo] hostName]];
    _netService = [[NSNetService alloc] initWithDomain:@"" type:@"_iKeyStrokes._tcp." name:serviceName port:self.listeningSocket.localPort];
...
-(void)messageBroker:(TBMessageBroker *)server didReceiveMessage:(IKMessage *)message{
    ...
    if ( (_delegate && [_delegate respondsToSelector:@selector(didReceiveMessageWithString:)])) {
        
        [_delegate didReceiveMessageWithString:message.string];
